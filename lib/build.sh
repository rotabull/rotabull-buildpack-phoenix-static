...
download_node() {
  ...
  if [ ! -f ${cached_node} ]; then
    ...
    if ! read number url < <(curl --silent --get --retry 5 --retry-max-time 15 --data-urlencode "range=$node_version" "https://nodebin.jxltom.com/v1/node/$platform/latest.txt"); then
